...
function HudClass() {
  ...
  this.freezeEnemies = function () {
    ...
    for (let i = 0; i < enemyList.length; i++) {
      ...
      if (
        (currEnemy.x - p1.x) ** 2 + (currEnemy.y - p1.y) ** 2 <=
        FREEZE_RADIUS ** 2
      ) {
  ...
  this.draw = function () {
    ...
    drawBitmapCenteredAtLocationWithRotation(
      healthIcon,
      this.heartX + spacing * 0,
      this.heartY,
      0
    );
    printText(
      this.currentHealth,
      this.printHealthX + spacing * 0,
      this.printHealthY,
      24,
      "white"
    );
    ...
    drawBitmapCenteredAtLocationWithRotation(
      boomstickIcon,
      this.heartX + spacing * 1,
      this.heartY,
      0
    );
    printText(
      this.currentAmmo,
      this.printAmmoX + spacing * 1,
      this.printAmmoY,
      24,
      "white"
    );
    ...
    drawBitmapCenteredAtLocationWithRotation(
      swordIcon,
      this.swordX + spacing * 2,
      this.swordY,
      0
    );
    printText(
      "[space]",
      this.printAmmoX + spacing * 1 + 50,
      this.printAmmoY - 64,
      16,
      "yellow"
    );
    ...
    const messagesYPosition =
      this.messageWindowY + this.messageWindowHeight / 4;
    ...
    for (let i = 0; i < messagesToDisplay && i <= messages.length - 1; i++) {
      printText(
        messages[i].message,
        this.messageWindowX + 20,
        messagesYPosition + messagesYGap * i,
        16,
        MessageColor[messages[i].type]
      );
    }
    drawRect(
      this.messageWindowX,
      this.messageWindowY,
      this.messageWindowWidth,
      this.messageWindowHeight,
      4,
      "white"
    );
  };
